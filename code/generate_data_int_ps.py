...
def archive():
    ...
    date_time = now.strftime("%Y%m%d_%H:%M:%S_int_ps")
...
while True:
    ...
    density = 1
    key_count = (choices(population=[1e6,1e7,1e8,1e9])[0])#floor(target_total_size / (payload_size + avg_key_size))
    #key_count=6000000
    ...
    try:
        try:
            ...
            stdout, stderr = process.communicate(timeout=90)  # timeout after 10 seconds
